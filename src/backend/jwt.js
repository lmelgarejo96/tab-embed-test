...
const tableauUserConnect = process.env.TABLEAU_USERNAME || ''

//  Get an authentication token using our connected app
router.get('/', (req, res) => {

    //    Decript the username, using our secure key
    const username = tableauUserConnect || tableauHelper.decryptUserId(req.query.encryptedUserId);
    ...
    let payload = {
        ...
        'scp': ['tableau:views:embed', 'tableau:metrics:embed', 'tableau:ask_data:embed', 'tableau:views:embed_authoring']    //    Scopes
    }
    let options = {
        'header': {
